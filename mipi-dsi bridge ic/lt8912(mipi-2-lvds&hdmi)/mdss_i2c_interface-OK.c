...
int Reset_chip(void)
{
	//gpio_direction_output(954, 1);
	//gpio_direction_output(1018, 1);
	// À­µÍLT8912 µÄreset pin£¬delay 150 ms×óÓÒ£¬ÔÙÀ­¸ß
...
int mdss_mipi_i2c_init(struct mdss_dsi_ctrl_pdata *ctrl, int from_mdp)
{
	...
	*/
	...
	//	MIPIDig1280x720(); 800x1280
		my_mipi_i2c->mdss_mipi_i2c_client->addr = 0x49;
		HDMI_WriteI2C_Byte(0x18,0x18); // hwidth
		HDMI_WriteI2C_Byte(0x19,0x02); // vwidth 6
		HDMI_WriteI2C_Byte(0x1c,0x20); // H_active[7:0]
		HDMI_WriteI2C_Byte(0x1d,0x03); // H_active[15:8]
		HDMI_WriteI2C_Byte(0x1e,0x67); // hs/vs/de pol hdmi sel pll sel
		HDMI_WriteI2C_Byte(0x2f,0x0C); // fifo_buff_length 12
		HDMI_WriteI2C_Byte(0x34,0x98); // H_total[7:0]
		HDMI_WriteI2C_Byte(0x35,0x03); // H_total[15:8]
		HDMI_WriteI2C_Byte(0x36,0x18); // V_total[7:0]
		HDMI_WriteI2C_Byte(0x37,0x05); // V_total[15:8]
		HDMI_WriteI2C_Byte(0x38,0x0A); // VBP[7:0]
		HDMI_WriteI2C_Byte(0x39,0x00); // VBP[15:8]
		HDMI_WriteI2C_Byte(0x3a,0x0C); // VFP[7:0]
		HDMI_WriteI2C_Byte(0x3b,0x00); // VFP[15:8]
		HDMI_WriteI2C_Byte(0x3c,0x18); // HBP[7:0]
		HDMI_WriteI2C_Byte(0x3d,0x00); // HBP[15:8]
		HDMI_WriteI2C_Byte(0x3e,0x48); // HFP[7:0]
	...
	//	void Collpll51M(void)// PLL 80.4MHz
		...
		HDMI_WriteI2C_Byte(0x69,0x14);
		HDMI_WriteI2C_Byte(0x69,0x94);
		HDMI_WriteI2C_Byte(0x6c,0x8A);
		HDMI_WriteI2C_Byte(0x6b,0x3D);

	//******************************************//

	//	LVDS Output¼Ä´æÆ÷ÉèÖÃ
	//	1366X768 800x1280
		my_mipi_i2c->mdss_mipi_i2c_client->addr = 0x48;
		HDMI_WriteI2C_Byte(0x80,0x00);
		HDMI_WriteI2C_Byte(0x81,0xff);
		HDMI_WriteI2C_Byte(0x82,0x03);
		HDMI_WriteI2C_Byte(0x83,0x20);
		HDMI_WriteI2C_Byte(0x84,0x03);
		HDMI_WriteI2C_Byte(0x85,0x80);
		HDMI_WriteI2C_Byte(0x86,0x10);
		HDMI_WriteI2C_Byte(0x87,0x98);
		HDMI_WriteI2C_Byte(0x88,0x03);
		HDMI_WriteI2C_Byte(0x89,0x18);
		HDMI_WriteI2C_Byte(0x8a,0x18);
		HDMI_WriteI2C_Byte(0x8b,0x02);
		HDMI_WriteI2C_Byte(0x8c,0x20);
		HDMI_WriteI2C_Byte(0x8d,0x00);
		HDMI_WriteI2C_Byte(0x8e,0x53);
		HDMI_WriteI2C_Byte(0x8f,0x00);
		HDMI_WriteI2C_Byte(0x90,0x10);
		HDMI_WriteI2C_Byte(0x91,0x00);
		HDMI_WriteI2C_Byte(0x92,0x10);
