...
{
    int ret = 0;
    printk("eliot:cxn_cdev_open success.\n");
    return ret;
}
...
{
    printk("eliot:cxn_cdev_release release success.\n");
    return 0;
}
...
static ssize_t cxn_cdev_read(struct file *filp, char *buf, size_t len, loff_t *off)
{
    ...
    if(copy_to_user(buf, r_value_buff, len))
    {
...
static ssize_t cxn_cdev_write(struct file *filp, const char *buf, size_t len, loff_t *off)
{
	...
	mutex_lock(&my_cxn_i2c->lock);
	ret = i2c_master_send(my_cxn_i2c->cxn_i2c_client,buffer,len);
	mutex_unlock(&my_cxn_i2c->lock);
...
static void cxn_work_func(struct work_struct *work)
{
	...
	ret = i2c_master_recv(cxn->cxn_i2c_client,r_value_buff,32);
	printk(KERN_ERR "eliot: ret = %d,value[0]=%d,value[1]=%d,value[2]=%d\n",ret,r_value_buff[0],r_value_buff[1],r_value_buff[2]);
	
	if(r_value_buff[0]==0 && r_value_buff[1]==1 && r_value_buff[2]==0)
	{
		cxn_open();
	...
	{
		/*report 0x04(event),0x03(code),oxfe(value) to notify app to read data from cx010x*/
		input_event(cxn_input_dev, EV_MSC, MSC_RAW, 0xfe);
		input_sync(cxn_input_dev);
	}
	if(r_value_buff[0]==0x11 && r_value_buff[1]==0x01 && r_value_buff[2]==0x80)
	{
		//into mute mode ,thermal protect 60'
		count_black++;
		printk(KERN_ERR "========eliot: into black screen mode %d !!!!======\n",count_black);
	}
	if(r_value_buff[0]==0x11 && r_value_buff[1]==0x01 && r_value_buff[2]==0x0)
	{
