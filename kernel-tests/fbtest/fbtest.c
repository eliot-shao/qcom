...
int parseFileName(const char *file, struct inputFileParams *thisInputFileParams)
{
	...
	if (filename == NULL)
		return -PARSE_FAILED;
	strlcpy(thisInputFileParams->filename, filename, sizeof(thisInputFileParams->filename));
...
static void print_user_args(struct fbtest_params *thisFBTEST)
{
	...
	for (i = 0; i < 4; ++i) {
		if (thisFBTEST->blend.ImagePath[i][0]) {
			VPRINT(verbose, "Input image [%d]:       %s\n",
					i, thisFBTEST->blend.ImagePath[i]);
		}
		else {
			VPRINT(verbose, "Input image [%d]:       None\n", i);
		}
	}
				...
				while((dit = readdir(dip)) != NULL) {
					...
					if (ptr) {
						if ((!strcmp(ptr, ".rgb")) || (!strcmp(ptr, ".yuv"))) {
							...
							strlcat(tmp,thisFBTEST->path,(strlen(thisFBTEST->path) + 1));
							strlcat(tmp,"/",(strlen(thisFBTEST->path) + 1));
							strlcat(tmp,dit->d_name,(strlen(thisFBTEST->path) + 1));
							VPRINT(verbose, "\nInput Image: %s\n", dit->d_name);
